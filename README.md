...
- Al desplegar el panel, el plasmoide comprueba como máximo una vez cada 24 horas si hay una versión nueva publicada, mediante una petición HTTPS a `api.github.com` que no envía credenciales ni identificadores de usuario (como en cualquier petición HTTPS, GitHub recibe la IP de origen y las cabeceras habituales). El límite de 24 horas se reinicia si se reinicia Plasma o se crea otra instancia del widget. Esta comprobación no se realiza si el panel no se abre.
...
./packaging/rpm/sign-package.sh dist/upclink-vpn-1.0.3-1.x86_64.rpm
```

El paquete resultante se genera en `dist/`.

RPM compilado y probado en openSUSE Tumbleweed actual (Qt 6.11); no probado en otras distribuciones.
...
1. `upclink-vpn-1.0.3-1.x86_64.rpm` — instalador de la aplicación.
2. `upclink-vpn-1.0.3-1.x86_64.rpm.sha256` — comprobación de integridad.
...
```bash
sha256sum -c upclink-vpn-1.0.3-1.x86_64.rpm.sha256
```
...
```bash
sudo zypper install ./upclink-vpn-1.0.3-1.x86_64.rpm
```
...
rpm -Kv dist/upclink-vpn-1.0.3-1.x86_64.rpm
sha256sum -c dist/upclink-vpn-1.0.3-1.x86_64.rpm.sha256
```
